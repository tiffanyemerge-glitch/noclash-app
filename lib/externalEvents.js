...
// whatever's already scheduled on Ticketmaster over the next couple of months in one call,
// so the board can slot each result under its own day instead of looping a request per date.
// Always resolves (never throws) — same contract as checkTicketmaster.
async function checkTicketmasterCity({ city, state, windowDays = 90, size = 40 }) {
  if (!ticketmasterConfigured()) {
    return { provider: 'Ticketmaster', configured: false, events: [], error: null };
  }
