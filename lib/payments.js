...
async function createListingCheckout({ plan, priceCents, eventName, successUrl, cancelUrl, metadata }) {
  return stripe().checkout.sessions.create({
    mode: 'payment',
    // No payment_method_types here on purpose: Stripe's Managed Payments (on by default for
    // newer accounts) now picks and manages the enabled payment methods itself, and rejects a
    // Checkout Session that still specifies payment_method_types explicitly — see
...
async function createPassCheckout({ successUrl, cancelUrl, metadata }) {
  return stripe().checkout.sessions.create({
    mode: 'payment',
    // See the note in createListingCheckout above — Managed Payments handles this now.
    line_items: [
      {
        price_data: {
          currency: 'usd',
