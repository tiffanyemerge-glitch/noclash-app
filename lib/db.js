...
async function updateEvent(id, organizerId, patch) {
  ...
  await pool.query(
    ...
    [id, organizerId, merged.name, merged.date, merged.startTime, merged.city, merged.state, merged.category, merged.link, merged.status, merged.expiresOn, merged.description || '']
  );
  return merged;
}
